...
The owner's stated bar (2026-09-22) is that character, background, cut and style consistency across a script is **non-negotiable**. A code review that day found the mechanism is real but partial — one thing is actually enforced in code, three things are only requested in a prompt and can silently drift.

**Enforced in code, not just asked for:**
- `styleAnchor`. `applyVisualDirection` (`server/scriptPipeline.ts`) takes the first non-empty `styleAnchor` produced by *any* art-direction chunk and force-overwrites every scene's `styleAnchor` with it (`canonicalAnchor`). This is genuinely byte-identical across the whole script regardless of what an individual chunk's call returned.
- **Fixed the same day:** the flat `visualPrompt` field — the one field every non-layered consumer reads, and what `README.md` calls "the always-present fallback" — used to be built from only `visual.scene + visual.styleAnchor`. It dropped `visual.character` (where a character's verbatim `promptAnchor` lives) and `visual.background` entirely, so anything reading `visualPrompt` instead of the full `visual` object got none of the consistency work below. Now built from `character + background + scene + styleAnchor` (`negative` stays out — separate field, an image API's negative-prompt parameter, never the positive prompt). A `NO_CHARACTERS_SENTINEL` constant keeps the model's "No characters in frame." placeholder out of that concatenation. Tests: `scriptPipeline.test.ts`, "visualPrompt is built from character + background..." and the sentinel-filtering test.

**Requested in the prompt, never verified — real gaps:**
- **`promptAnchor` reuse.** `generateVisualDirectionChunk`'s prompt says to reuse each character's anchor "copied word for word, unchanged," but nothing checks that a returned `visual.character` string actually contains it. Unlike `styleAnchor`, there is no code-level override — a paraphrase or a dropped clause passes silently.
- **Recurring backgrounds.** `background` is free text, per scene, describing "ONLY the environment." If the story returns to a location already shown in an earlier, different art-direction chunk (chunks are 6 scenes each, `VISUAL_DIRECTION_SCENES_PER_CHUNK`), that later chunk's call has no visibility into the earlier chunk's exact wording — nothing asks for consistency here, let alone enforces it. The narrative pass's rolling context (`priorScenesContext`, `generateSceneChunks`) only carries the last 3 scenes' narration forward, not any visual/background detail, and the art-direction pass gets no rolling context at all between chunks.
- **Cut/shot rhythm.** `motion.shotType` / `cameraMove` / `transitionOut` are written per scene with no cross-chunk awareness of how earlier scenes were shot. The only related audit (`timeline.ts`'s `no-pattern-interrupt` check) covers `visualType` monotony for retention pacing, not camera or cut variety. Documentary tone gets a loose per-chunk guideline ("terminal/diagram/headline for at least half the scenes") that is never tallied across the full script.

None of these three are built yet. Whoever picks this up next: the natural fix for the first is the same shape as `styleAnchor`'s enforcement — verify (or force) `promptAnchor` inclusion in code inside `applyVisualDirection`, not by asking the model harder. The other two need the art-direction pass to see more of the script than its own 6-scene chunk, which runs into the same large-schema/large-context tradeoffs documented throughout "Things that will bite you" above — don't just widen the chunk size without retesting live first.
...
- **Enforcement:** `SEVERITY_RATING_RE` in `server/timeline.ts` matches the word CVE or CVEs, a CVE id, or CVSS. The audit raises `severity-rating-shown` as a **warn**, because only a rewrite can fix it and CyberPipe's review shows warnings. `lintTitle` and `lintThumbnail` treat it as an **error**, so the publish package's one retry kicks in.
